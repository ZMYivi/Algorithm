...
就是个模拟题，首先需要明确任务，就是要找到第n位数字，这个第n为指的是把数字看成字符串，一个数字算一位，比如10就算是两位。因此需要先确定n是属于哪一个数字的，然后再确定属于这个数字中的那一个数字。就是一个大模拟的状态，按照数字的位数遍历，然后计算位置就完事了。
...
这种第几小或者大的大部分题目都会用到大根堆或者小根堆，c++提供了一个标准库优先队列，只需要重载一下运算符，因为分数比较，先同分再进行比较大小，然后就没了。虚假的困难题。
